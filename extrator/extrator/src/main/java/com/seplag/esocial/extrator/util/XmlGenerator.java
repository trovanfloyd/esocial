...
public class XmlGenerator {
	...
	public Document testGenXml(Map<String, Object> xmlValues, String xsdPath, String evento) throws Exception {
		...
		// QName elName = new
		// QName("http://www.esocial.gov.br/schema/evt/evtInfoEmpregador/v02_04_02",
		// "eSocial");
		QName elName = new QName("http://www.esocial.gov.br/schema/evt/" + evento + "/v02_04_02", "eSocial");
		...
		/*
		 * NodeList list = rootElement.getChildNodes(); NodeList list2 =
		 * rootElement.getElementsByTagName("ds:Transform");
		 * setString("ds:Transform", rootElement, "123456");
		 *
		 * if (list != null && list.getLength() > 0) { if
		 * (list.item(5).getNodeType() == Node.ELEMENT_NODE) { Element el =
		 * (Element) list.item(5); if
		 * (el.getNodeName().contains("ds:Signature")) { //
		 * setString("ds:DigestValue", el, //
		 * "CFJEIy1dUko99nNUW/ICvG9ZNoij0o9IOhdP6Nt1j1k="); } }
		 *
		 * NodeList subList = list.item(0).getChildNodes(); if (subList != null
		 * && subList.getLength() > 0) {
		 *
		 * } }
		 */

		setId(xmlValues, rootElement);

		xmlValues.forEach((k, v) -> {
			if (v != null) {
				System.out.println(k + " " + v.toString());
				setString2(BASE_XSD + k, rootElement, v.toString());
			}
		});

		// setString2("nmRazao", rootElement, "testetetetettetet");

		// setString(BASE_XSD + "nrSiafi", rootElement, s1000.getNrSiafi());
		// String requestQueueName = getString(BASE_XSD + "nrSiafi",
		// rootElement);
		 ...
		 * DOMSource(document), new StreamResult(writer)); String output =
	...
	private void setId(Map<String, Object> xmlValues, Element rootElement) {
		id = xmlValues.get("id").toString();
	...
	protected void setString2(String tagName, Element element, String value) {
